        ...
        if (data && data.length > 0) {
          ...
          const transformedReviews = reviews.map(
            (review: OutscraperReview) => ({
              ...
              publishTime:
                review.review_datetime_utc || new Date().toISOString(),
              relativePublishTimeDescription:
                review.review_timestamp || "Recently",
            }),
          );

          transformedReviews.sort(
            (a: TransformedReview, b: TransformedReview) => {
          ...
          return {
            ...
            totalRatings:
              businessData.reviews_count || transformedReviews.length,
          };
        }
      } else {
